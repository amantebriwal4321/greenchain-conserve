...
const ComponentLabel: React.FC<ComponentLabelProps> = ({ 
  opacity, side, xOffset, yOffset, title, description, color, isDark = false 
}) => {
    ...
    >
      <div className="relative h-px w-24 md:w-28 overflow-visible">
        ...
        >
          ...
          />
          <circle cx="0" cy="5" r="3" fill={colorMap[color]} />
        </svg>
      </div>
      ...
      <div className={`px-4 py-1 flex flex-col ${side === 'left' ? 'mr-1' : 'ml-1'}`}>
        ...
        <p className="text-neutral-500 text-[9px] leading-tight max-w-[140px] font-bold uppercase tracking-wider opacity-80">
          {description}
        </p>
      </div>
    </motion.div>
